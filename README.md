...
**Note:** TinyMCE text editor was used for the main story field, however there is a issue with this module that causes the full rich text editor to fail to display in production. I attempted a number of fixes to this in my last several commits, however I only discovered this issue shortly before the submission deadline, and owing to the relative complexity of the issue in question, I was unable to fix this before submission. It works as expected in a local development environment, and fixing this in production would be a priority for future development.
